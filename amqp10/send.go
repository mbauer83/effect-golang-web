...
func Send[R any](link SenderLink, message Message) effect.Effect[R, Fault, effect.Unit] {
	return effect.Try(
...
func SendValue[R, A any](
	...
	return effect.Try(
		...
		func(err error) Fault { return faultOf("encode a message", link.Address(), err) },
	).
