...
// receiverLink is a receiver link and the deliveries it has not settled.
...
func newReceiverLink(receiver *broker.Receiver, address string) ReceiverLink {
	return &receiverLink{
		receiver:  receiver,
		address:   address,
		unsettled: map[string]*broker.Message{},
...
func (link *receiverLink) Address() string { return link.address }
...
func (link *receiverLink) Receive(ctx context.Context) (Delivery, bool, error) {
	...
	delivery, err := deliveryOf(message)
	if err != nil {
		return Delivery{}, false, err
	}
	...
	link.unsettled[delivery.Tag] = message
	return delivery, true, nil
}

func (link *receiverLink) Accept(ctx context.Context, tag string) error {
...
func (link *receiverLink) Reject(ctx context.Context, tag string, reason string) error {
	return link.settle(ctx, tag, func(ctx context.Context, message *broker.Message) error {
...
func (link *receiverLink) Release(ctx context.Context, tag string) error {
	return link.settle(ctx, tag, func(ctx context.Context, message *broker.Message) error {
		return link.receiver.ReleaseMessage(ctx, message)
	})
}

func (link *receiverLink) Modify(ctx context.Context, tag string, change Change) error {
	...
	return link.settle(ctx, tag, func(ctx context.Context, message *broker.Message) error {
		return link.receiver.ModifyMessage(ctx, message, &broker.ModifyMessageOptions{
			DeliveryFailed:    change.DeliveryFailed,
			UndeliverableHere: change.UndeliverableHere,
			Annotations:       annotations,
		})
	})
}

func (link *receiverLink) Close(ctx context.Context) error {
...
func (link *receiverLink) settle(
	ctx context.Context,
	tag string,
	disposition func(context.Context, *broker.Message) error,
) error {
	link.mutex.Lock()
	message, found := link.unsettled[tag]
...
// ignoreClosed treats an already-closed connection, session or link as the
